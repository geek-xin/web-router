...
| `localIp` | 可选本地监听 IP；空值且配置本地端口时默认 `127.0.0.1`。 |
...
| 匹配方式 | 按每个 `pathPrefixes` 注册 Gateway Path 谓词 | 先按当前路由 `pathPrefixes` 做入口隔离 |
| 前缀处理 | 会按路径层级 `StripPrefix` | 不剥离前缀 |
| 转发路径 | 剥离前缀后的路径 | 原始请求 URI |
| 未命中路径 | 不命中该 Gateway 路由 | 返回 `404`，不会转发 |
...
- Gateway 转发到目标服务 `/hello`。
- 本地端口代理转发到目标服务 `/test/hello`。
